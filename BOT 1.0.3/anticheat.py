            ...
            if count>=5:
                ...
                if dt.check('robot')==False:
                    ...
                    if dt.check('busy_fox')==True and dt.check('sign')==False:
                        ...
                        sleep(1.5)
                    dt.find('sign')
                    historic.printt('Captcha | Successful')
                    return(True)
                else:
                    sleep(1)
                    if dt.find('sign')==True:
                        ...
                        return(True)
                    if e<3:
                        historic.printt('Captcha | Reseted')
                        return(detect_puzzle(e+1))
                    else:
                        cs.ctrl(cs.F5)
                        historic.printt('Captcha | Failed')
                        return(False)
...
def test():
    L=[(728, 845),
     ...
     (2070, 845),
     (1071, 1186)]
    L2=[(1807, 740), (541, 845), (542, 845), (1757, 845), (1758, 845)]
    for i in range(len(L2)):
        cs.move(L2[i][0],L2[i][1])
        sleep(3)
