...
def captcha():
    ...
    if len(L)>=1:
        ...
        fg=find_numbers_fg(xm,xM,yrobot)
        #print('fg: ',fg)
        #SETTING UP BAR SIZE
        ...
        found=False
        for k in range(4):
            cs.pyautogui.moveTo(xmcurseur+(k+1)*xk,ycurseur,0.01)
            cs.random_sleep(0.5)
            bg=find_numbers_bg(xm,xM,yrobot,0,N)
            #print('bg: ',bg)
            if bg==fg:
                found=True
                cs.random_sleep(0.5)
                cs.pyautogui.move(0,2,0.01)
                cs.mouse.release(cs.Button.left)
                cs.random_sleep(1)
                break

        if found==False:
            cs.pyautogui.moveTo(xmcurseur,ycurseur,0.01)
            cs.random_sleep(0.5)
            cs.pyautogui.move(0,2,0.01)
            cs.mouse.release(cs.Button.left)
            cs.random_sleep(1)
...
def find_numbers_bg(xm,xM,yrobot,e,N):
    take_data(xm,xM,yrobot,10)
    numbers_found=[]
    for i in range(10):
    ...
    if len(numbers_found)!=3 and e<=5:
        return(find_numbers_bg(xm,xM,yrobot,e+1,N))
    return(numbers_found)
#NEEDS TO BE ON FG SCREEN
def find_numbers_fg(xm,xM,yrobot):
    take_data(xm,xM,yrobot,1)
    found_couples=[]
    j=0
...
def take_data(xmin,xmax,yrobot,N):
    if 0<=xmin<=xmax<=1281:
        if yrobot<=719:
            x1,y1,x2,y2=300,271,769,484
        else:
            x1,y1,x2,y2=300,947,769,1160
    elif 1281<=xmin:
        if yrobot<=719:
            x1,y1,x2,y2=1415,271,1884,484
        else:
            x1,y1,x2,y2=1415,947,1884,1160
    else:
        x1,y1,x2,y2=300,271,769,484
    if N==1:
