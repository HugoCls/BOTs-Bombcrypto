...
def sweepScreen(m):
    im = captureMss(m)
    res = locate("images/Reveal Number.png", im, confidence=0.8)
    img_bwo = None
    goAhead = False
    if (res != None):
        ...
        return cv2.cvtColor(morph_img, cv2.COLOR_BGR2RGB)

def slideAndDrop(m,number):
    im = captureMss(m)
    captchaPos = locate("Recognition/captchaBar.png", im, confidence=0.7)
    if (captchaPos != None):
        robotPos = locate("Recognition/robot.png", im, confidence=0.8)
        newArea = {
        ...
        if(robotPos != None):
            ...
            for x in range(captchaX, robotX, 7):
                ...
                yRange = int(random.uniform(captchaY - 6, captchaY + 6))
