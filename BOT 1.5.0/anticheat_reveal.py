...
def captcha():
    ...
    if len(L)>=1:
        ...
        xk=round((xMcurseur-xmcurseur)/7)
        found=False
        fg=[]
        for k in range(1,16):
            if xmcurseur+(k+1)*25<xMcurseur+10:
                cs.pyautogui.moveTo(xmcurseur+(k+1)*25,ycurseur,0.01)
                if k>=1:
                    old_fg=fg
                    for i in range(3):
...
def add_non_black_pixels(full_img,added_img):
    ...
    #cv2.imshow('Changed img',added_img)
    #cv2.waitKey(0)
    return(full_img)

#Has to be taken for a new module called image processing
